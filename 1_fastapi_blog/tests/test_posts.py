...
async def test_create_post_success(client: AsyncClient):
    ...
    data = response.json()
    # checking it return correct response structure
...
async def test_create_post_unauthorized(client: AsyncClient):
    response = await client.post(
        ...
        # here we didn't pass auth header for testing unauthorized create post
    )
...
async def test_update_post_success(client: AsyncClient):
    ...
    # first create post and then call update 
    response = await client.post(
        "/api/posts",
...
async def test_get_posts_with_pagination(client: AsyncClient):
    '''create multiple post and asserting that they get posts return paginated response'''
    await create_test_user(client)
    token = await login_user(client)
    headers = auth_header(token)
