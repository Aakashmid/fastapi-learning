...
from utils.image_utils import process_profile_image, delete_profile_image , upload_profile_image
...
async def forget_password(
    ...
    if user:
        print("user exist")
        # delete old reset token to prevent any future issue
...
async def reset_password(request_data:ResetPasswordRequest,db: Annotated[AsyncSession, Depends(get_db)],):
    ...
    if reset_token.expires_at.replace(tzinfo=UTC) < datetime.now(UTC):  # here tzinfo used to convert model object expires at to it can be compared to datetime.now
...
async def get_user_posts(
    ...
    # check whether user exist or not before fetching posts
    result = await db.execute(select(models.User).where(models.User.id == user_id))
    user = result.scalars().first()
    if not user:
        raise HTTPException(
...
async def update_profile_picture(
    ...
    # this line is unnecceosry and we don't need to use user_id in api as we have to just update image of current user
    #  also test after your own changes (removing user_id use)
    if current_user.id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to update this user profile picture",
        )
    ...
    try:
        processed_content,new_filename = await run_in_threadpool(process_profile_image, content)
    ...
    try:
        await upload_profile_image(processed_content,new_filename)
    ...
    # first save changes then delete old file so if any error occur old file remain
    await db.commit()
